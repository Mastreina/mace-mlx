...
> **状态：已完成（2026-07-04）。** 结果报告见
> `docs/prototypes/sparse_sc_results.md`。最终方案是 U 的 (i,k) 行压缩
> 双线性 GEMM（§4.1 说的「最低成本路径」的胜出形式），custom VJP 实测
> 对 force-only 反向无增益、未采用。全部验收达标：块级 f+b 2.7×/内存
> 4×，端到端 1.3-1.7×，medium/Si2000 峰值 37.7→21.2 GB，dF 4.5e-06，
> 640 测试全过。原文保留如下，供背景参考。
